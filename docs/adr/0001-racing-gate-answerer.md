...
- Detection is per session for `sessions`/`subagent`/`all` and per turn for `scheduled`/`cron`, the latter read off the session's last user message — "is anyone watching", not "was this ever machine-driven".
